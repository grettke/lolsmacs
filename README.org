...
Emacs will start loading a lot of files earlier than you've probably seen before. Sometimes, that slows down the startup too much, and it doesn't make sense to load ~lolsmacs~ right away. That is fine; it still works later, too—it's just /ideal/ to load right away.
